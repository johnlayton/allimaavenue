(function ( root, factory ) {
  ...
  NetCDFParser.prototype.readFloatLE = function () {
    return this.slice( 4 ).readFloatLE( 0 )
  };
    ...
    while ( proccessing ) {
      switch ( parser.state ) {
        ...
        case S.DATA_VAR:
          ...
          parser.debug( "Data shape", util.inspect( parser.temp.shape ) );
          parser.state = S.DATA_VAR_READ;
          break;
        case S.DATA_VAR_READ:
          var take = parser.temp.type == 5 ? function ( idx ) {
            return parser.options.treat( parser.readFloat() );
          } : function ( idx ) {
            return parser.options.treat( parser.readInt32() );
          };
          var arr = new Float32Array( parser.length / 4 );
          for ( var i = 0; i < ( parser.length / 4 ); i++ ) {
            arr[i] = take( i );
          }
          variable = {
            ...
            type     : parser.temp.type,
            data     : ndarray( arr, parser.temp.shape )
          };
